...
export const getI18nProps = async (ctx, ns = ['common']) => {
  ...
  const props = {
    locale: locale || i18nextConfig.i18n.defaultLocale,
...
export const makeStaticPropsWithPosts = (ns = []) => async (ctx) => {
  const locale = ctx?.params?.locale;
  const postsDir = locale ? `public/locales/${locale}/posts` : 'public/locales/en/posts';
  const announDir = locale ? `public/locales/${locale}/announcements` : 'public/locales/en/announcements';
  return {
    props: {
      ...(await getI18nProps(ctx, ns)),
      posts: JSON.parse(JSON.stringify(getAllPosts(postsDir))),
      announcements: JSON.parse(JSON.stringify(getAllAnnouncements(announDir)))
    }
  };
};
